...
페이로드 32KB→16MB 스윕. end-to-end latency를 **queue / GPU compute / serialize+transport+H2D·D2H(other)** 로 분해(ms):
...
도출되는 주요 병목:
- **Serialization/Network overhead**: 작은 페이로드에서 `other`가 지배. protobuf encode/decode + 2-hop TCP + H2D/D2H 복사가 합쳐진 값. 1MB에서 46ms로 최대 구성요소.
- **GPU bottleneck**: 큰 페이로드에서 compute가 급증(16MB에서 146ms)하여 **transport-bound → compute-bound 크로스오버** 발생.
- **Queue contention / Synchronization overhead**: 워커 단일 stream을 mutex로 직렬화하므로 동시 요청이 워커에서 대기 → `queue_ms` 증가(4MB에서 97ms). 워커 스케일링이 sub-linear한 근본 원인(단일 GPU 공유).
- **CPU**: OpenMP gather/scatter는 memcpy 바운드이나 전체 대비 작아 주 병목 아님.

> 참고: `other`는 직접 계측이 아니라 `latency − queue − compute`의 잔여값이라 직렬화/전송/복사가 합산되어 있다. 4-way 정밀 분해(H2D/D2H 별도 `cudaEvent` + proto 필드)는 향후 과제.
