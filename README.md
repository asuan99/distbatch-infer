...
**Roofline (ncu)** — `results/roofline.png`. Confirms the intended split:

| kernel | SM/compute % | DRAM % | classification |
|---|---|---|---|
| gemm  | 88.1 | 1.1  | compute-bound |
| gelu  | 60.8 | 78.6 | memory-bound |
| softmax | 28.0 | 80.5 | memory-bound |

**GPU batch scaling** (`results/batch_scaling.png`) — samples/s rises 8.4k (b=1)
-> ~51k (b=256) and saturates as the GPU fills; requests/s falls since each
request is larger.

**Sequence-length scaling** (`results/seqlen_scaling.png`) — mean latency
2.7ms (S=128) -> 9.7 -> 26.9 -> 89.6ms (S=2048); grows faster than linear
(attention is O(S²)).

**Worker scaling** (`results/worker_scaling.png`) — throughput 462 -> 588 ->
595 req/s for 1/2/4 workers (≈1.0 / 1.27 / 1.29×). Sub-linear **by design**:
all workers share one physical GPU, so the gain comes from H2D/compute/D2H
overlap across processes, not extra compute.
...
**Bottleneck breakdown** (`results/breakdown.png`, batch=8 S=256, 1MB request) —
mean e2e split: queue 20.1ms / GPU compute 18.2ms / serialize+transport+H2D·D2H
39.7ms. The 1MB protobuf payload makes serialization/transport the largest
component — the documented payload-size bottleneck.
